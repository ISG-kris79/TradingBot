...
namespace TradingBot
{
    public partial class MainWindow : Window
    {
        ...
        // [Phase 14] UI 이벤트 핸들러
        private async void BtnStartArbitrage_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (_arbitrageService == null)
                {
                    AddAlert("❌ 차익거래 서비스가 초기화되지 않았습니다.");
                    return;
                }

                // 설정 업데이트
                if (decimal.TryParse(txtArbitrageMinProfit.Text, out decimal minProfit))
                {
                    _arbitrageService.Settings.MinProfitPercent = minProfit;
                }
                if (int.TryParse(txtArbitrageScanInterval.Text, out int scanInterval))
                {
                    _arbitrageService.Settings.ScanIntervalSeconds = scanInterval;
                }

                var symbols = new List<string> { "BTCUSDT", "ETHUSDT", "SOLUSDT" };
                await _arbitrageService.StartAsync(symbols, _advancedFeaturesCts?.Token ?? default);

                btnStartArbitrage.IsEnabled = false;
                btnStopArbitrage.IsEnabled = true;
                txtArbitrageStatus.Text = "상태: 실행 중 🟢";
                txtArbitrageStatus.Foreground = new SolidColorBrush(Color.FromRgb(16, 185, 129));
                AddLog("✅ 차익거래 서비스 시작됨");
            }
            catch (Exception ex)
            {
                AddAlert($"❌ 차익거래 시작 실패: {ex.Message}");
            }
        }

        private void BtnStopArbitrage_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                _arbitrageService?.Stop();
                btnStartArbitrage.IsEnabled = true;
                btnStopArbitrage.IsEnabled = false;
                txtArbitrageStatus.Text = "상태: 중지됨";
                txtArbitrageStatus.Foreground = new SolidColorBrush(Color.FromRgb(100, 116, 139));
                AddLog("🛑 차익거래 서비스 중지됨");
            }
            catch (Exception ex)
            {
                AddAlert($"❌ 차익거래 중지 실패: {ex.Message}");
            }
        }

        private async void BtnStartFundTransfer_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (_fundTransferService == null)
                {
                    AddAlert("❌ 자금 이동 서비스가 초기화되지 않았습니다.");
                    return;
                }

                // 설정 업데이트 (필요시)
                await _fundTransferService.StartMonitoringAsync(_advancedFeaturesCts?.Token ?? default);

                btnStartFundTransfer.IsEnabled = false;
                btnStopFundTransfer.IsEnabled = true;
                txtFundTransferStatus.Text = "상태: 실행 중 🟢";
                txtFundTransferStatus.Foreground = new SolidColorBrush(Color.FromRgb(16, 185, 129));
                AddLog("✅ 자금 이동 서비스 시작됨");
            }
            catch (Exception ex)
            {
                AddAlert($"❌ 자금 이동 시작 실패: {ex.Message}");
            }
        }

        private void BtnStopFundTransfer_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                _fundTransferService?.Stop();
                btnStartFundTransfer.IsEnabled = true;
                btnStopFundTransfer.IsEnabled = false;
                txtFundTransferStatus.Text = "상태: 중지됨";
                txtFundTransferStatus.Foreground = new SolidColorBrush(Color.FromRgb(100, 116, 139));
                AddLog("🛑 자금 이동 서비스 중지됨");
            }
            catch (Exception ex)
            {
                AddAlert($"❌ 자금 이동 중지 실패: {ex.Message}");
            }
        }

        private async void BtnStartRebalancing_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (_rebalancingService == null)
                {
                    AddAlert("❌ 리밸런싱 서비스가 초기화되지 않았습니다.");
                    return;
                }

                // 설정 업데이트 (필요시)
                await _rebalancingService.StartMonitoringAsync(_advancedFeaturesCts?.Token ?? default);

                btnStartRebalancing.IsEnabled = false;
                btnStopRebalancing.IsEnabled = true;
                txtRebalancingStatus.Text = "상태: 실행 중 🟢";
                txtRebalancingStatus.Foreground = new SolidColorBrush(Color.FromRgb(16, 185, 129));
                AddLog("✅ 리밸런싱 서비스 시작됨");
            }
            catch (Exception ex)
            {
                AddAlert($"❌ 리밸런싱 시작 실패: {ex.Message}");
            }
        }

        private void BtnStopRebalancing_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                _rebalancingService?.Stop();
                btnStartRebalancing.IsEnabled = true;
                btnStopRebalancing.IsEnabled = false;
                txtRebalancingStatus.Text = "상태: 중지됨";
                txtRebalancingStatus.Foreground = new SolidColorBrush(Color.FromRgb(100, 116, 139));
                AddLog("🛑 리밸런싱 서비스 중지됨");
            }
            catch (Exception ex)
            {
                AddAlert($"❌ 리밸런싱 중지 실패: {ex.Message}");
            }
        }
