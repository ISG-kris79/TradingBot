...
$projectPath = Join-Path $scriptRoot "TradingBot\Mobile\CoinFF.Mobile.csproj"

if (-not (Test-Path $projectPath)) {
    Write-Host "❌ Mobile project file not found: $projectPath" -ForegroundColor Red
    exit 1
}
...
if ([string]::IsNullOrWhiteSpace($AndroidSdkDirectory)) {
    ...
    foreach ($candidate in $candidateSdkPaths) {
        if (Test-Path $candidate) {
            $AndroidSdkDirectory = $candidate
            break
        }
    }
}
...
if (-not [string]::IsNullOrWhiteSpace($AndroidSdkDirectory)) {
    Write-Host "📁 Using Android SDK at: $AndroidSdkDirectory" -ForegroundColor Gray
    $publishCommand += " -p:AndroidSdkDirectory='$AndroidSdkDirectory'"
}
else {
    Write-Host "⚠️ Android SDK path not detected. Install Android SDK or pass -AndroidSdkDirectory." -ForegroundColor Yellow
}
...
$apkPublishDir = Join-Path $scriptRoot "TradingBot\Mobile\bin\$Configuration\net9.0-android\publish"
$apkPath = Get-ChildItem -Path $apkPublishDir -Filter "*-Signed.apk" -ErrorAction SilentlyContinue | Select-Object -First 1

if ($apkPath) {
    Write-Host "📦 APK created at: $($apkPath.FullName)" -ForegroundColor Yellow

    # [추가] 버전 정보 읽기 및 파일명 변경
    [xml]$csprojXml = Get-Content $projectPath -Raw
    $version = $csprojXml.Project.PropertyGroup.ApplicationDisplayVersion | Select-Object -First 1

    if ([string]::IsNullOrWhiteSpace($version)) {
        $version = $csprojXml.Project.PropertyGroup.ApplicationVersion | Select-Object -First 1
    }
