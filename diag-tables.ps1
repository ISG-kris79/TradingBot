. "$PSScriptRoot\diag-since-noon.ps1" -DryRun 2>$null
function Get-CS {
...
AND (TABLE_NAME LIKE '%Trade%' OR TABLE_NAME LIKE '%Position%' OR TABLE_NAME LIKE '%Order%' OR TABLE_NAME LIKE '%Log%' OR TABLE_NAME LIKE '%Closed%' OR TABLE_NAME LIKE '%History%')
ORDER BY TABLE_NAME
