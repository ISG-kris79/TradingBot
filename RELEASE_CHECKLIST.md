...
배포 시작 스크립트를 사용하면 체크리스트를 자동으로 확인하고 필요한 명령어를 제공합니다:

```powershell
# 배포 도우미 스크립트 실행
.\start-release.ps1 -Version "X.X.X"
```

**이 스크립트가 자동으로 해주는 것:**
- ✅ RELEASE_CHECKLIST.md를 열어줌
- ✅ 배포 단계별 체크리스트 표시
- ✅ 필요한 명령어를 복사 가능한 형태로 제공
- ✅ 원라인 배포 명령어 생성 및 클립보드 복사

**배포 시 두 가지 자동 확인 시스템:**
1. `publish-and-release.ps1` 실행 시 자동으로 체크리스트 확인 프롬프트 표시
2. Git 태그 푸시 시 pre-push hook이 체크리스트 확인 알림
