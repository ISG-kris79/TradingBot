...
Write-Host "=== TRAILING/SL/TP 에러 상세 (최근 6h) ===" -ForegroundColor Cyan
Q @"
SELECT TOP 20 Timestamp, LEFT(Message, 300) AS Msg
FROM FooterLogs
WHERE (Message LIKE '%errCode%' OR Message LIKE '%errMsg%' OR Message LIKE '%TRAILING_STOP%실패%'
       OR Message LIKE '%[SL]%실패%' OR Message LIKE '%[TP]%실패%'
       OR Message LIKE '%STOP_MARKET 실패%' OR Message LIKE '%TAKE_PROFIT%실패%')
