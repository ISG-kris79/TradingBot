...
Write-Host "=== [1] FIGHTUSDT 오늘 매매기록 ===" -ForegroundColor Cyan
Q @"
...
FROM TradeHistory WHERE Symbol='FIGHTUSDT' AND EntryTime >= '2026-04-14'
ORDER BY UserId, EntryTime ASC
"@ | Format-Table -AutoSize

Write-Host "=== [2] FIGHTUSDT 5분봉 (08:00~14:30) ===" -ForegroundColor Cyan
Q @"
SELECT OpenTime, [Open], High, Low, [Close],
    CAST(([Close]-[Open])/[Open]*100 AS decimal(6,2)) AS ChgPct
FROM CandleData WHERE Symbol='FIGHTUSDT' AND IntervalText='5m'
  AND OpenTime >= '2026-04-14 08:00:00' AND OpenTime <= '2026-04-14 14:30:00'
ORDER BY OpenTime ASC
"@ | Format-Table -AutoSize

Write-Host "=== [3] FIGHTUSDT 유저1 진입 로그 ===" -ForegroundColor Cyan
Q @"
...
FROM FooterLogs WHERE Id > (SELECT MAX(Id)-100000 FROM FooterLogs)
  ...
  AND (Message LIKE '%ENTRY%START%' OR Message LIKE '%BLOCK%' OR Message LIKE '%FILLED%' OR Message LIKE '%RR%')
ORDER BY Id ASC
"@ | Format-Table -AutoSize
