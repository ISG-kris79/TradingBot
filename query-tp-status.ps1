...
Write-Host "=== SL/TP/Trailing 등록 로그 ===" -ForegroundColor Cyan
Q @"
...
FROM FooterLogs WHERE Id > (SELECT MAX(Id)-3000 FROM FooterLogs)
  ...
  AND (Message LIKE '%성공%' OR Message LIKE '%실패%' OR Message LIKE '%등록%')
ORDER BY Id DESC
"@ | Format-Table -AutoSize

Write-Host "=== UserId=1 활성 포지션 ===" -ForegroundColor Cyan
Q @"
SELECT Symbol, EntryPrice, Quantity, Strategy, EntryTime, IsClosed
FROM TradeHistory WHERE UserId=1 AND IsClosed=0
ORDER BY EntryTime DESC
"@ | Format-Table -AutoSize

Write-Host "=== Order_Error ===" -ForegroundColor Cyan
Q "SELECT TOP 5 * FROM Order_Error ORDER BY Id DESC" | Format-Table -AutoSize
