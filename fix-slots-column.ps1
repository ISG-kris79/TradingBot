...
IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.GeneralSettings') AND name = 'MaxMajorSlots')
    ALTER TABLE dbo.GeneralSettings ADD MaxMajorSlots INT NOT NULL DEFAULT 4;
IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.GeneralSettings') AND name = 'MaxPumpSlots')
    ALTER TABLE dbo.GeneralSettings ADD MaxPumpSlots INT NOT NULL DEFAULT 3;
"@
$cm.ExecuteNonQuery()
Write-Host "MaxMajorSlots, MaxPumpSlots columns added"
$cn.Close()
