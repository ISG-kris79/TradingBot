...
Write-Host "=== 최근 차단 로그 ===" -ForegroundColor Cyan
Q @"
SELECT TOP 15 Id, Timestamp, LEFT(Message, 280) AS Msg
FROM FooterLogs WHERE Id > (SELECT MAX(Id)-3000 FROM FooterLogs)
  AND (Message LIKE '%BLOCK%' OR Message LIKE '%차단%')
ORDER BY Id DESC
"@ | Format-Table -AutoSize

Write-Host "=== 최근 진입 시도 ===" -ForegroundColor Cyan
Q @"
SELECT TOP 10 Id, Timestamp, LEFT(Message, 280) AS Msg
FROM FooterLogs WHERE Id > (SELECT MAX(Id)-3000 FROM FooterLogs)
  AND (Message LIKE '%ENTRY%START%' OR Message LIKE '%FILLED%' OR Message LIKE '%주문요청%')
ORDER BY Id DESC
"@ | Format-Table -AutoSize
