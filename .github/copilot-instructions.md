...
이 레포는 Windows WPF 기반의 트레이딩 봇입니다. UI 엔트리포인트는 `TradingBot/MainWindow.xaml(.cs)`이며, 실제 매매/분석 로직은 `TradingBot/TradingEngine.cs`에 집중되어 있습니다. AI·ML 관련 코드는 `MLService.cs`, `AITrainer.cs`, `AIPredictor.cs`, `AiInferenceService.cs`에 흩어져 있으며 ONNX 및 ML.NET 기반 추론·학습을 모두 사용합니다.
...
- 출력 폴더에 `model.onnx` 및 ML.NET `.zip` 모델이 있어야 추론이 정상 동작합니다.
...
- ML/AI:
  ...
  - ONNX 추론: `AiInferenceService.cs` (model.onnx, ONNX Runtime 사용, 입력 shape 및 전처리 중요)
- 통합(알림): `TelegramService.cs` — Telegram.Bot 이용, 봇 토큰/채팅 ID 하드코딩됨

## 데이터 흐름(요약)
1. Binance WebSocket에서 실시간 캔들/틱 데이터 수신 (`BinanceSocketClient`)
2. `TradingEngine`이 지표(RSI, Bollinger 등) 계산 및 `AiInferenceService`/`AIPredictor` 호출
3. 결정 결과는 UI(`MainWindow.Instance.RefreshSignalUI`)와 DB(`DbManager`)에 반영
4. 주문 실행은 `TradingEngine`에서 Binance REST/Trading API 호출

## 프로젝트·운영상의 중요 관찰/규칙
- 모델 파일명 불일치: 프로젝트 내에 `model.onnx`, `Model.zip`, `TradingModel.zip`, `market_model.zip` 등 여러 이름이 등장합니다. 코드에서 사용하는 정확한 파일명(예: `AIPredictor`는 `Model.zip`, `MLService`는 `TradingModel.zip`)을 확인하고 통일해야 합니다.
- ONNX 경로: `AiInferenceService`는 `AppDomain.CurrentDomain.BaseDirectory`로 `model.onnx`를 찾습니다 — 실행 시점의 출력 디렉터리에 파일이 있어야 합니다.
...
## 의존성(주요 NuGet)
- `Binance.Net`, `Microsoft.ML`, `Microsoft.ML.OnnxRuntime`, `Microsoft.Data.SqlClient`, `Dapper`, `Telegram.Bot`, `LiveCharts.Wpf` — 확인: `TradingBot/TradingBot.csproj`

## 개발자용 팁 & 예시
- ONNX 추론 디버깅: 브레이크포인트를 `AiInferenceService` 생성자에 걸고 `Console.WriteLine`으로 모델 경로를 확인(이미 코드에 출력 있음).
- 학습 재현: `AITrainer.TrainAndSave(List<CandleData>)` 또는 `MLService.Train(...)`을 호출해 로컬에서 `.zip` 모델 생성 가능.
...
- ML/모델 작업: `AiInferenceService.cs`(ONNX 텐서 shape, 전처리) 및 `MLService.cs`/`AITrainer.cs`(학습 파이프라인)
- DB 관련: `DbManager.cs`에서 연결 문자열과 주요 쿼리 확인
- 통합/배포: `TradingBot/TradingBot.csproj`에서 Output 설정(`model.onnx`가 CopyToOutputDirectory로 설정됨)을 확인
