...
Write-Host "=== UserId=1 IsClosed=0 ===" -ForegroundColor Cyan
Q @"
...
Write-Host "=== MAX 슬롯 설정 ===" -ForegroundColor Cyan
Q @"
SELECT TOP 5 Id, Timestamp, LEFT(Message, 200) AS Msg
FROM FooterLogs WHERE Id > (SELECT MAX(Id)-3000 FROM FooterLogs)
  AND (Message LIKE '%total=%' OR Message LIKE '%MAX_TOTAL%' OR Message LIKE '%Active:%')
ORDER BY Id DESC
"@ | Format-Table -AutoSize
