...
Write-Host "=== [1] 币安人生USDT 매매기록 ===" -ForegroundColor Cyan
Q @"
...
FROM TradeHistory WHERE Symbol LIKE '%人生%' AND EntryTime >= '2026-04-14'
ORDER BY UserId, EntryTime ASC
"@ | Format-Table -AutoSize

Write-Host "=== [2] 5분봉 ===" -ForegroundColor Cyan
Q @"
SELECT TOP 20 OpenTime, [Open], High, Low, [Close],
    CAST(([Close]-[Open])/[Open]*100 AS decimal(6,2)) AS ChgPct
FROM CandleData WHERE Symbol LIKE '%人生%' AND IntervalText='5m'
  AND OpenTime >= DATEADD(HOUR, -2, GETDATE())
ORDER BY OpenTime ASC
"@ | Format-Table -AutoSize

Write-Host "=== [3] 진입 로그 ===" -ForegroundColor Cyan
Q @"
SELECT TOP 20 Id, Timestamp, LEFT(Message, 280) AS Msg
FROM FooterLogs WHERE Id > (SELECT MAX(Id)-50000 FROM FooterLogs)
  AND Message LIKE N'%人生%'
  AND (Message LIKE '%ENTRY%' OR Message LIKE '%BLOCK%' OR Message LIKE '%FILLED%' OR Message LIKE '%SIGNAL%')
ORDER BY Id ASC
"@ | Format-Table -AutoSize
