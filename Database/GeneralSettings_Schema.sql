...
-- GeneralSettings 테이블 생성 (사용자별)
CREATE TABLE dbo.GeneralSettings (
    Id INT IDENTITY(1,1),
    UserId INT NOT NULL,  -- 사용자별 구분
    ...
    CONSTRAINT PK_GeneralSettings_UserId PRIMARY KEY (UserId, Id),
    CONSTRAINT FK_GeneralSettings_Users FOREIGN KEY (UserId) REFERENCES Users(UserId) ON DELETE CASCADE
);

-- 인덱스 생성 (사용자별 조회 최적화)
CREATE INDEX IX_GeneralSettings_UserId ON dbo.GeneralSettings(UserId DESC, UpdatedAt DESC);

-- 초기 데이터 삽입 (각 사용자별 기본 설정)
-- 참고: Users 테이블에 이미 사용자가 있어야 합니다
-- INSERT INTO dbo.GeneralSettings (UserId, DefaultLeverage, DefaultMargin, TargetRoe, StopLossRoe, TrailingStartRoe, TrailingDropRoe)
