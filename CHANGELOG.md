...
### 📉 15분봉 단타 스펙 3종 측정 — 81조합 전부 적자, 라이브 미반영

사용자 지시 스펙을 `--scalp15` 로 그대로 구현해 3년·30코인 측정:
A. 20/60 EMA 추세 눌림목 · B. 볼린저 스퀴즈 돌파 · C. RSI 다이버전스 역추세.
리스크 원칙 반영 — 손익비 1:1.5 이상 · 보유 4시간 상한 · 손절폭 0.2~6% · 왕복 0.08%.
축: 방향필터(없음/1h/1h+4h) × 익절(RR1.5/RR2.0/스펙 동적) = 27조합, 감도 3회 = **81 측정**.

```
A.EMA눌림목   1h+4h  RR2.0   73,234건  37.5%  -0.121R  PF0.81   -8,834R  전폴드 음수
B.BB스퀴즈돌파 1h+4h  RR2.0    9,932건  36.8%  -0.117R  PF0.81   -1,166R  전폴드 음수
C.RSI다이버전스 1h+4h RR2.0    1,447건  38.4%  -0.115R  PF0.79     -166R  전폴드 음수
```

**엣지 부재이지 비용 문제가 아니다** — 수수료 0 으로 재측정해도 27조합 전부 PF 0.91~1.00
(최고가 A/1h+4h/RR2.0 의 +0.002R·PF1.00 = 동전던지기). 보유상한을 24시간으로 늘려도 동일.
교과서 표준 셋업이라 이미 차익거래로 소진된 것으로 보인다.

부수 확인 3가지:
- **1h+4h 방향필터는 세 전략 모두 개선**(A −0.138→−0.121R). 건수는 1/3 로 줄지만 방향 필터 자체는 유효.
- **RR2.0 > RR1.5** 일관.
- **스펙의 동적 익절이 고정 RR 보다 항상 나쁘다.** C 의 20EMA 도달 익절은 승률 66.6% 인데 −0.260R·PF0.17 —
  전형적인 "작은 이익 여러 번 + 큰 손실 한 번". 기존 확정 "조기청산은 직관과 반대로 손실↑"(v5.25.30)과 일치.
- BB 상단돌파(B)의 적자는 기존 확정 룰 "BB 상단돌파 진입 절대금지"(2026-06-09)를 독립적으로 재확인한다.

비교: 현재 채택된 엘리엇 구조는 **+0.089R**(수수료 포함·5폴드 전부 양수). 15분봉 단타 3종은 건당 0.2R 이상 열등.
**→ 라이브 배선 없음. 백테스트 모드만 추가.**

### 🔁 이전 검증 — 단타 발견 2종을 엘리엇 흑자구조에 얹어봤으나 둘 다 채택 불가

`--elliott-spec` 을 개편(카운터 1회 패스 + 방향필터·손익비 축 동시 측정)해 한계효과를 쟀다.

**① 손익비 확대 = 곡선맞춤.** RR 2.0~6.0 을 촘촘히 훑으면 RR4.0 이 총 131R 로 최고지만,
폴드별 최적 RR 이 5.0 / 2.0 / 2.0 / 4.0 / 2.5 로 전부 다르다 — 안정적 최적점이 없다.
차이(0.087~0.111R)도 n=1,190 기준 표준오차(±0.04) 안이다. 게다가 **최근 폴드일수록 높은 RR 이 죽는다**
(RR3.5·4.0 에서 5번째 폴드 0.00, RR5.0 부터 음수). 라이브는 그 최근 구간의 연장이므로 총R 최고점
선택은 정확히 반대다. → **RR 3.0 유지.**

**② 상위프레임 방향필터 = 건당 품질을 사고 총액을 판다.**
```
현행(필터없음) 1,193건 +0.089R 106R · 최근200봉 2건 · 5폴드 전부 양수
+1h             535건 +0.105R  56R · 최근200봉 1건 · 5폴드 전부 양수
+1h+4h          297건 +0.140R  42R · 최근200봉 0건 · 4번째 폴드 -0.14
```
단타에서 본 "1h+4h 가 건당 R 을 올린다"는 엘리엇에서도 재현되나(+0.089→+0.140R), 진입이 75% 잘려
총R 이 106R→42R 로 줄어든다. 1h+4h 는 5폴드 안정성 실패 + **숏 소멸(145→5건·-0.616R)** +
**최근 200봉 신호 0건(무진입 재발)**. 1h 단독도 총R 반토막. → **미채택.**

→ 라이브 코드 변경 없음. 측정 도구만 개선.

## [5.34.15] - 2026-08-25

### 🧊 무진입의 진짜 원인 = 파동 카운터 영구 동결 (라이브 3일 무변화 입증 + 해소)

사용자 지적: "등락폭도 큰데 진입이 없다는게 말이 안 된다." 맞다. 규칙이 드문 게 아니라 **카운터가 얼어 있었다.**

**라이브 로그 실측** (`log-20260822~25`, 시딩 08-22 12:36 KST):

| 시각 | ADA | SOL | BTC | 처리봉 |
|---|---|---|---|---|
| 08-22 12:46 | 2파^ / 되78% | 2파^ / 되42% | 4파^ | 1500 |
| 08-25 13:28 | 2파^ / 되78% | 2파^ / 되42% | 4파^ | **1790** |

290봉(3일)이 정상 공급됐는데 **21종목 전부 상태가 한 글자도 안 바뀌었다.** `★신호포착:19` 도 사흘 내내 19 고정.
진입 신호는 파동2가 *마감되는 순간*에만 발생하므로, 파동이 안 끝나면 진입은 구조적으로 0건이다.

**기전** — 파동 마감(`bos`)은 `bigEnough`(레그폭 ≥ `_minLegPct` 0.4%)를 전제로 하는데 `_legExt` 는
레그 방향으로만 갱신된다. 레그가 0.4% 를 못 채운 채 반대로 돌아서면 `legLen` 이 영구 고정 →
`bigEnough` 가 영영 false → **파동이 절대 끝나지 않는다.** 추적 극점이 반대편에 있어 시장이 아무리
움직여도 스스로 안 풀린다. **등락폭이 클수록 더 갇힌다.**

**신설 `--elliott-frozen`** — 라이브 시딩을 그대로 재현해 봉마다 내부값을 찍는다.
실측: 16종목 중 **13종목이 291봉 완전 동결·신호 0건**. 같은 구간 시장 등락폭 SOL 18%·XRP 23%·LINK 26%.

### 📊 3년·30코인 측정으로 갈린 것 — 동결 해제만으로는 적자가 실현될 뿐

```
현행라이브 피보마감+피보익절 (v5.34.13)  12,794건  24.6%  -0.363R  PF0.67   -4,646R  전폴드 음수
  └ 동결만 해제                        117,251건  22.7%  -0.597R  PF0.55  -70,029R  전폴드 음수
프랙탈K21 마감 + 3배TP                   1,206건  34.8%  +0.087R  PF1.14     +105R  ★전폴드 양수
```

같은 3배TP·같은 무효화에서 **마감축만** 바꾼 비교가 병목을 지목한다 — 피보 23.6% 마감은 파동을
너무 일찍 끊어 **가짜 파동2 마감**을 양산한다(승률 34.8% → 23.6%, 11%p 붕괴).
`termK` 중간구간(5·8·10·12·14·17·26)도 전수 스윕했으나 K21 을 못 넘었다(K17 +88R, 나머지 전부 적자).

### ✅ 채택 (사용자 승인 2026-08-25)

- **마감축 = 프랙탈 K21 복귀** (`TermModeDefault` 1 → 0). `bigEnough` 는 `termMode==1` 전용이므로
  **동결이 구조적으로 사라진다.** 재현 확인: 16종목 15개 상태 진행, 3일 6건 ≈ **하루 2건 진입**.
- **익절 = 진입가 ± 3×손절폭 (1:3) 복귀.** v5.34.13 의 1.618 피보 확장투영은 +40R(5폴드 2개 음수)로
  3배TP(+105R·전폴드 양수)에 못 미쳤다. 승률은 피보TP 가 높지만 목표가 가까워 건당 R 이 잘린다.
- **손절폭 범위(0.2~6%) 복귀** — 채택된 흑자 구조는 이 필터가 켜진 상태로 측정된 값이다.
- **사용자 스펙 보강 2종 신규 구현** (`SpecStructDefault` = true):
  - **5파 절단(Truncation)** — 파동5에 무효화 분기가 **아예 없어** 4파 종점을 깨고 내려가도 영원히
    '5파 진행중'으로 남던 결함을 메운다. 3파 종점 미돌파 + 4파 종점 이탈 = 추진파 절단 완성.
  - **엔딩 다이아고날** — 4파가 1파를 침범해도 수렴(3파 < 1파)이면 추진파 무효가 아니라 다이아고날.
    카운트를 버리지 않고 5파로 넘긴다.
  - 한계효과: 1,193건 +0.089R PF1.14 **총 +106R · 5폴드 전부 양수** (롱 총R 81R → 90R).

### ⛔ 미채택 (측정 반증)

- **구조 재해석(지그재그 C파 / degree 승격 / ABC 전환, `specInval`)** — 사용자 스펙이지만 3년 측정에서
  +105R → **−42R**(승률 34.8% → 32.6%). 다만 현행 라이브 규칙 위에서는 진입을 12,794 → 1,549건으로
  줄여 손실을 1/7 로 낮췄다 — 쓰레기 진입을 거르는 효과 자체는 실재한다. 스위치로 보존.
- **미달레그 흡수(`AbsorbMicroLegDefault` = false)** — 동결을 푸는 정확한 수정이지만 프랙탈 마감에는
  데드락이 없어 불필요하고, 얹으면 +105R → +48R(5폴드 2개 음수). 피보 마감 재사용 대비 스위치로 보존.

## [5.34.14] - 2026-08-22

### 🔍 확정 — 라이브는 12번 진입했어야 한다 (배선 결함 입증)

사용자 지적: "계속 급등인데 안 들어가냐? 15분봉이면 진입타점이 새벽에 나왔는데." 맞다.

**신설 `--elliott-seedsim`** — 라이브와 **완전히 동일한 순서**로 재현한다.
① 봇 재시작 시각(08-21 22:54 KST) 직전 1500봉으로 시딩 → ② 이후 마감봉을 하나씩 Advance.
콜드스타트(`--elliott-state`)와 나란히 비교해 '경로 의존성' 핑계를 배제한다.

| 심볼 | 재시작 후 신호 |
|---|---|
| ADA | 03:15 · 06:00 · 06:45(S5숏) · 08:15 · 10:15 — **5건** |
| BCH | 08-21 23:15 · 01:30 · 02:15(S5숏) · 06:30 · 08:30 — **5건** |
| DOT | 07:00 · 10:15 — **2건** |

**합계 12건.** ADA·BCH 는 추적풀에도 있었다. 라이브 실제 진입은 **0건**.
경로 의존성이 아니라 **라이브 배선 결함**이다.

### ⚠️ 원인 미확정 — 계측으로 좁힌다

단계요약에 `★진입`·`게이트차단`·`피보목표초과`·`♻️`가 **전부 0건**이라
"신호가 잡히긴 했는가"조차 로그로 확인할 수 없었다. 그 사각을 없앴다.

검토해 **기각한** 가설(기록):
- 다봉 따라잡기 중 신호 소멸 → 심볼당 분석 주기가 **2초**(`SYMBOL_ANALYSIS_MIN_INTERVAL_MS`)라
  15분봉을 2봉 이상 놓칠 수 없다. 원인 아님(안전장치로는 채택).
- 레이트리밋/캔들부족/분석오류 → 라이브 로그 실측 **각 0건**.
- 되돌림 밴드·얼어붙음 → v5.34.13 정상 작동 중(파동2가 30~78%로 순환).

### Added — 계측

- `★신호포착` 단계 + `🌊 [ELLIOTT] {심볼} 신호포착 | LONG/SHORT {종류} 손절/목표/되돌림/현재가`
  — 신호가 잡힌 사실 자체를 무조건 남긴다. 이후 어느 분기로 죽든 추적 가능.
- `🌊 [ELLIOTT] 파동상태` — 10분마다 **심볼별** 파동/방향/되돌림/처리봉수.
  같은 시각 `--elliott-state`·`--elliott-seedsim` 과 1:1 대조용.
- 무로그로 빠져나가던 구멍 보강: `롱:현재가가손절선이하` / `숏:현재가가손절선이상`
  (종전엔 값도 안 남기고 '신호없음'으로 잘못 집계됐다)

### Fixed — 실재하는 위험 2건 제거 (이번 원인은 아님)

- 진입임박 패널(`UpdateNearEntryForSymbolAsync`, 50초 주기·고정 16종목)이 진입 판정과
  **같은 `_ewCounters`** 를 `Advance()` 하고 있었다. 진입창 0이라 패널이 한 봉 더 밀면
  진입 루프는 신호를 영영 못 본다 → 패널 전용 `_ewCountersView` 로 분리.
- 전진 루프가 끝난 뒤 신호를 **한 번만** 읽던 것을 **봉마다** 집도록 변경
  (재시작·캐시 정체 등 다봉 따라잡기 상황 대비. 시뮬레이터와 동일한 읽기 순서).

## [5.34.13] - 2026-08-21

### ★ 엘리엇 파동 + 피보나치 **단독** 진입으로 정정 (사용자 지시)

사용자 지적: "엘리엇파동 피보나치수열 단독으로 진입하라고 했는데 이상한거 또 해놨다."
라이브 진입 경로를 전수 점검한 결과, 엘리엇도 피보나치도 아닌 조건이 6개 붙어 있었다.

**제거/교체한 비(非)스펙 조건**

| 위치 | 제거한 것 | 비고 |
|---|---|---|
| `TradingEngine.cs` | 마감봉 대비 1% 급변 시 스캔 포기 | 라이브 5시간 17,765회 발동 |
| `TradingEngine.cs` | 신호봉 종가 대비 0.15% 슬립컷 | |
| `TradingEngine.cs` | 손절폭 0.2~6% 범위 강제 | |
| `TradingEngine.cs` | `SIDEWAYS_BOX` 게이트 | 엘리엇 소스 면제 |
| `TradingEngine.cs` | `BTC_1H_DOWNTREND` 게이트 | 엘리엇 소스 면제 — BTC 레짐게이트는 금지 규칙(코인 디커플링) |
| `ElliottWaveEngine.cs` | 파동 마감 = 프랙탈 K21 → **피보 되돌림 23.6%** | `TermModeDefault` 0 → 1 |
| `ElliottWaveEngine.cs` | 익절 = 고정 3×손절폭 → **피보 확장투영** | 롱 1.618×파동1 · 숏 0.618 되돌림 |

`SetupBos.TargetPrice` 신설 — 1.618 확장목표를 계산만 하고 버리던 것을 실제 TP로 연결.
투영이 이미 초과된 자리는 구조상 여유가 없으므로 진입하지 않는다(`피보목표초과`).

### 📊 신설 `--elliott-spec` — 스펙을 2×2로 분해 측정 (3년·30코인·5폴드)

| 설정 | 건수 | 승률 | 기대R | PF | 총R | 최근신호 | 5폴드 |
|---|---|---|---|---|---|---|---|
| 구라이브 프랙탈+3배TP | 1,206 | 34.8% | +0.087 | 1.14 | **+105R** | 2건 | 전부 양수 |
| 마감만 피보(23.6%)+3배TP | 9,121 | 23.6% | −0.205 | 0.77 | −1,869R | 19건 | 전부 음수 |
| 익절만 피보(1.618)+프랙탈 | 1,526 | 37.7% | +0.026 | 1.04 | +40R | 2건 | 3/5 양수 |
| **★신스펙(채택)** | 12,794 | 24.6% | −0.363 | 0.67 | **−4,646R** | **20건** | 전부 음수 |

**무진입은 해소된다** — 진입 1,206 → 12,794건(10배), 최근 200봉 신호 2 → 20건.
프랙탈 마감이 상승추세에서 얼어붙던 문제(v5.34.12 규명)가 원인부터 제거됐다.
라이브 실측(2026-08-21 5시간)에서 무신호 스캔 169,073 중 69%가 '파동1 미마감'이었다.

**단, 3년 백테 총R은 +105R → −4,646R 로 무너진다.** 변수 분리 결과 범인은 익절축이
아니라 마감축이다(익절만 교체 시 +40R로 거의 중립). 마감 비율을 38.2%/50%로 키워도
−5,012R / −3,387R 로 회복되지 않는다. "빈도를 늘리면 총R이 무너진다"의 21번째 재확인.

이 손실 측정을 보고한 뒤 사용자가 배포를 지시하여 스펙대로 채택했다.

## [5.34.12] - 2026-08-21

### 🔍 규명 — "XRP 29% 상승 동안 진입 0건" 의 직접 원인 = 카운터가 급등장에서 얼어붙는다

**가격으로 증명** (`--elliott-state` 에 앵커/1파끝 가격 노출):

```
[XRPUSDT]  08-19 18:30  1파v  종가=1.0033  앵커=1.0094   ← '하락 1파' 시작
           08-20 17:15  2파v  종가=1.1408  앵커=1.0094   ← +13.7% 오르는 내내 '하락 1파'
[ETHUSDT]  08-19 17:45  1파v  종가=1920.75 앵커=1929.8
           08-21 00:45  2파v  종가=2325.12 앵커=1929.8   ← +21% 오르는 내내 '하락 1파'
```

**기전** — 하락 레그의 마감 판정은 `프랙탈 고점(좌우 21봉보다 높은 고점) 돌파` 다.
상승 추세에서는 매 봉이 신고가라 **우측 21봉 조건을 만족하는 고점이 영영 확정되지 않는다**
→ 기준선(`_ref`) 미형성 → `bos` 미발생 → 파동이 안 끝난다.
**= 시장이 가장 크게 오를 때 카운터가 정확히 얼어붙는다.**
게다가 `specMix=1`(롱 전용)이라 하방앵커 구조에서는 롱 진입 후보가 **구조적으로 0** 이다.

### ❌ 반증 — 얼어붙음을 고치는 축 11종 전부 손실 (`--elliott-freeze`, 3년·30코인·5폴드)

| 설정 | 건수 | 총R | 역방향% | 5폴드 |
|---|---|---|---|---|
| **현행 프랙탈K21** | 1206 | **+105R** | 39.4% | **★전폴드양수** |
| 프랙탈K13 / K8 | 2046 / 3203 | −29R / −122R | 37.3% / 35.0% | 붕괴 |
| 되돌림마감 14.6~50% | 8528~9863 | **−1229R ~ −1980R** | 46.8~50.3% | 전폴드음수 |
| 되돌림+역행0.4% | 9562~9804 | −1707R / −1756R | 49.8% | 전폴드음수 |
| 현행+역행0.8% | 1327 | +55R | **30.0%** | 붕괴 |

`역방향%`(96봉 가격변화와 Dir 이 어긋난 봉 비율)를 39%→30% 로 낮춘 유일한 설정도
총R 이 반토막(105→55)나고 폴드 부호가 깨진다. **카운터를 시장 방향에 맞추면 엣지가 사라진다** —
v5.34.3(방향뒤집기 −110R) · v5.34.8(1파 역행무효화 53~79R) 에 이어 세 번째 독립 반증.
v5.34.4 가 이 얼어붙음 때문에 만들어둔 `termMode=1`(되돌림비율 마감)이 왜 기본값이 아닌지도 확정됐다.

### ❌ 반증 — "상승 중 눌림에 들어가기" 9종 전부 손실 (`--elliott-resume`)

현행은 **파동2 마감 봉 한 지점**에서만 진입하고 파동3/5 진행 중의 눌림은 전부 무시한다.
v5.34.4 가 구현해둔 `entryMode 1/2`(눌림 재개 진입)를 처음으로 판정했다:

| 설정 | 월건수 | 최근200봉 신호 | 총R | 롱총R |
|---|---|---|---|---|
| **현행(파동2 마감만)** | 33.6 | 2건 | **+105R** | +81R |
| +3파중 눌림 K3~K13 | 148~316 | 15~21건 | −38R ~ −783R | −42R ~ −467R |
| +3파중 눌림 K21 | 126 | 10건 | 0R | −24R |
| +3·5파중 눌림 K5~K21 | 132~257 | 9~17건 | −29R ~ −327R | −65R ~ −373R |

**진입 빈도는 5~10배가 되고 이번 8/19~8/21 급등도 실제로 잡는다(신호 2건→21건).
그런데 3년 총R 이 전부 마이너스다.** 눌림 재진입은 "잡히긴 하나 지는" 자리다.

### ⚠️ 부수 발견 — 재시작 시각에 따라 파동 카운트가 달라짐 (`--elliott-jitter`)

재시작하면 그 시점의 1500봉으로 처음부터 다시 센다. 시작 봉을 0~23봉(0~6시간) 밀어보면
**16종목 중 6종목이 다른 상태로 수렴**한다(ETH 상방71%/하방29% · BCH 상방33%/하방67%).
다만 최근 200봉 신호수는 0~1건으로 동일 → **이번 무진입의 원인은 아니다.** 별건으로 기록.

### Added
- `--elliott-freeze` — 얼어붙음 해소 축 스윕(termK·termMode·termRetr·w1WrongWay)
- `--elliott-resume` — 파동3/5 진행중 눌림 재개 진입 스윕(entryMode×minorK)
- `--elliott-jitter` — 재시작 시각 의존성 측정
- `--elliott-state` 에 앵커/1파끝 가격 표시, `--ew-syms` 로 심볼 지정
- `WaveCounter.AnchorPx/W1End/W2End/LegExt` 진단 노출 (판정 로직 불변)

### 결론 (사용자 판단 필요)
엘리엇 파동2 규칙은 **눌림이 완성된 자리**만 잡는다. 눌림 없이 수직으로 가는 구간은
정의상 못 잡고, 잡게 만드는 모든 방법이 3년 실측에서 돈을 잃는다.
급등 참여가 목표라면 **엘리엇을 고치는 게 아니라 별도 추세 슬리브가 필요**하며,
이는 v5.34.7 "엘리엇 단일 진입 · 복원 금지" 지시를 되돌리는 결정이라 사용자 승인이 필요하다.
