...
- **1분봉 하락추세 조기청산 필터 전면 제거** (PositionMonitorService)
  - ROE -26~-36% 구간에서 강제 청산하던 로직
  - 기본 손절(-40%) 도달 전 반등 기회 박탈이 주원인
  - 12시 이후 이 필터로 인한 손실: ENJUSDT -$9, BR -$8, BIO -$8, TAO -$7, XAG -$7 등 다수
  - 대안: 거래소 SL(-40% ROE) + 기본 손절 로직만 유지

### Added

- **BUY_PRESSURE 3분 타임아웃 본절 청산**
  - 데이터 분석 결과 BUY_PRESSURE 성공 전부 1분 내 체결
  - 3분 넘게 가는 BUY_PRESSURE는 거의 다 손실 → 빠른 탈출
  - 조건: 진입 3분 경과 + ROE < 5% + ROE > -10% (손절 구간은 건드리지 않음)
  - 효과: 오래 버티는 손실 포지션 조기 회수 (Scalping 모드)

### 예상 효과 (12시~현재 262건 -$186 기준)

- 1분봉 필터 제거 → **반등 기회 보존** → 조기 손절 ~40건 감소 (-$50 손실 방지)
- BUY_PRESSURE 타임아웃 → 3분+ 보유 손실 포지션 즉시 회수 (-$30 손실 방지)
- 합산 기대: **약 +$80 개선** (일일 PnL -$186 → -$106 수준)
...
- v5.9.10과 동일한 내용 (Velopack 인덱스 손상 복구를 위한 재빌드)
