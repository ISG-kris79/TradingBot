...
### 🔍 규명 — 신호는 밤새 났는데 라이브는 0건 진입

사용자 지적: "계속 급등인데 안 들어가냐? 15분봉이면 진입타점이 새벽에 나왔는데." 맞다.

`--elliott-state` 로 라이브와 동일한 `WaveCounter` 를 현재 시세에 재생한 결과 —
**16종목 15.6일 112건**(XLM 27 · ADA 24 · HYPE 13 · AVAX 10 …), 그중 08-22 새벽~오전만:

| 시각(KST) | 심볼 | 신호 |
|---|---|---|
| 03:15 | ADA | LONG retr 46.9% |
| 06:00 | ADA · XLM | LONG |
| 06:45 | ADA | S5 SHORT |
| 07:00 | XLM | LONG |
| 08:15 | ADA · XLM | LONG |
...
같은 시간 라이브(v5.34.13, uptime 13h)는 **진입 0건**. ADA·XLM 모두 추적풀에 있었고 스캔도 돌았다.
단계요약에 `★진입`·`게이트차단`·`피보목표초과`·`지연과다`가 **하나도 없다** =
라이브 카운터가 신호를 아예 못 냈다는 뜻. v5.34.13 의 얼어붙음 해소 자체는 정상 작동 중
(파동2가 30~78% 되돌림으로 순환).

### Fixed — 표시 전용 패널이 진입 카운터를 공유하던 위험 제거

`UpdateNearEntryForSymbolAsync`(진입임박 패널, 50초 주기·고정 16종목)가 진입 판정과
**같은 `_ewCounters` 인스턴스**를 `Advance()` 시키고 있었다. `EntryWindowBarsDefault=0` 이라
신호는 다음 `Advance` 한 번에 소멸하므로, 패널이 한 봉 더 전진시키면 진입 루프는 신호를 영영 못 본다.
→ 패널 전용 `_ewCountersView` 로 완전 분리.

※ 단, 이번 무진입의 **확정 원인은 아니다** — 그 경우라면 v5.34.11 의 신호회수 로그(`♻️`)가
찍혀야 하는데 0건이다. 공유 변경이라는 실제 위험을 제거한 것이지 원인 규명은 아직이다.

### Added — 원인 특정용 계측

- `🌊 [ELLIOTT] 파동상태` — 10분마다 **심볼별** 파동/방향/되돌림/처리봉수를 그대로 출력.
  같은 시각 `--elliott-state` 의 '현재 N파 앵커X' 와 1:1 대조하면 라이브 카운터가 어디서
  갈라지는지 바로 나온다. 종전 단계요약은 문자열 버킷 집계라 어느 심볼인지 알 수 없었다.
- 롱 경로에서 `riskL <= 0` 이 **무로그로 빠져나가** '신호없음'으로 집계되던 구멍 보강
  (`롱:현재가가손절선이하` 단계 신설).
